...
//리뷰 전체 정보 받기
export const postEvaluation = async (applicant_id: number, 
...
export const getEvalOthers = async (applicant_id: number) => {
  try {
...
//지원자 평가 현황 조회
export const getPassFail = async (applicant_id: number) => {
  try {
      const response = await axios.get(
...
//파트장이 결정하는 합불 결과
export const patchLeaderPassFail = async (applicant_id: number, accessToken: string,
  result: string) => {
  try {
